	...
	for (Object object : (List) images) {
		...
		for(Object innerPhraseArray:phraseObjectList){
			...
			for(Object phraseArray:innerObjectList){
			    ...
			    xywhMap.put(xywh, termCount);
			    LOG.info("termCount " + termCount);
			    
			    
			    String annoUrl = "madeUpResource"+xywh.toString();
			    annoURLMap.put(xywh.toString(), annoUrl);
			    resources.add(createResource(id, query, isW3c, xywh,annoUrl)); 
			    innerCount++;
			}			
		    }
		    count++;
